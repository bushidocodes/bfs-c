...
struct __attribute__((__packed__)) hdri
{ //header of internode message
	ushort routing;
	ushort sz;
	char hndl;
...
void aml_poll_intra(void)
{
	...
	if (flag)
	{
		...
		if (length > 0)
		{ //no confirmation & processing for ack only messages
...
SOATTR int aml_init(int *argc, char ***argv)
{
	...
	{
		printf("AML: Fatal: non power2 groupsize unsupported. Define macro PROCS_PER_NODE_NOT_POWER_OF_TWO to override\n");
		return -1;
	}
